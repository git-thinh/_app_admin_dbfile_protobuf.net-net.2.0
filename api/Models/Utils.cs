...
namespace api
{
    public class Utils
    {
        ...
        static IAuthorizationState GetStoredCredentials(String userId)
        {
            ...
            string file = Path.Combine(HostingEnvironment.ApplicationPhysicalPath, @"App_Data\" + userId + ".bin");
            if (System.IO.File.Exists(file))
            {
                StoredCredentials sc = null;
                using (var stream = new FileStream(file, FileMode.Open))
                {
                    sc = new BinaryFormatter().Deserialize(stream) as StoredCredentials;
                }
                return new AuthorizationState() { AccessToken = sc.AccessToken, RefreshToken = sc.RefreshToken };
            }
            return null;
        ...
        static void StoreCredentials(String userId, IAuthorizationState credentials)
        {
            ...
            {
                new BinaryFormatter().Serialize(stream, sc);
                stream.Flush();
                buf = stream.ToArray();
            }
