...
namespace Deveck.Ui.Controls
{
    public class CustomListView : ListView
    {
        ...
        public CustomListView(bool hideScrollBar = false)
        {
        ...
        {
            this.Clear();
            foreach (var pd in fields)
                this.Columns.Add(new ColumnHeader
                {
                    Text = pd.NAME,
                    Width = pd.Type.Name == "String" ? 250 : 70,
            ...
            foreach (object o in dataSource)
            {
                ...
                {
                    object val = o.GetType().GetProperty(cols[i]).GetValue(o, null);
                    //items[i] = Convert.ToString(fields[i].GetValue(itm));
                    items[i] = val == null ? string.Empty : val.ToString();
                }
                //items[fields.Length] = string.Empty;
        ...
        {
            ShowScrollBar(this.Handle, 1, false); // alway hide scrollbar SB_VERT
            if (_hideScrollBar)
                ShowScrollBar(this.Handle, 0, false); // hide scrolbar SB_HORZ
