...
namespace app.GUI
{
    public class FormModelEdit : FormBase
    {
        ...
        public delegate void EventSubmit(DB_MODEL model, bool hasRemoveField);
        ...
        private readonly DB_MODEL Model;
        ...
        private void init_Control()
        {
            ...
            TextBoxCustom txt_Name = new TextBoxCustom() { Left = 124, Top = 7, Width = 120, Text = Model.NAME, WaterMark = "Model name ...", BorderStyle = BorderStyle.FixedSingle, TextAlign = HorizontalAlignment.Center };
            ...
            TextBoxCustom txt_Caption = new TextBoxCustom() { Left = 372, Top = 7, Width = 120, Text = Model.CAPTION, WaterMark = "Model caption ...", BorderStyle = BorderStyle.FixedSingle, TextAlign = HorizontalAlignment.Center };
            ...
            field_Load(boi_Filter);
        }

        private void field_Load(FlowLayoutPanel boi_Filter)
        {
            foreach (var fi in Model.FIELDS.OrderBy(x=>x.ORDER_EDIT))
            {
                countField++;
                var uc = new ucModelFielEdit(countField, db, fi) { Name = "uc" + countField.ToString(), Height = 35, Width = fWidth };
        ...
        private void form_Submit(string dbName, string dbCaption, FlowLayoutPanel boi_Filter)
        {
            ...
            var fieldRoot = db.GetFields(dbName).Select(x => x.NAME).ToList();
            var li = new List<FieldInfo>();
            int index = 1;
            bool hasRemoveField = false;
            foreach (Control c in boi_Filter.Controls)
            {
                ...
                foreach (Control fi in c.Controls)
                {
                    ...
                    {
                        o.CAPTION_SHORT = fi.Text;
                    }
                    else if (fi.Name == "des" + index.ToString())
                    {
                        o.DESCRIPTION = fi.Text;
                    }
                    ...
                    {
                        int _vi = 0;
                        if (int.TryParse(fi.Text, out _vi))
                            o.ORDER_EDIT = _vi;
                    }
                ...
                if (!string.IsNullOrEmpty(o.NAME) && o.Type != null)
                {
                    ...
                    //if (o.FieldChange == dbFieldChange.REMOVE && fieldRoot.IndexOf(o.Name) == -1)
            ...
            if (li.Count > 0)
            {
                ...
                if (OnSubmit != null) OnSubmit(m, hasRemoveField);
            }
            else
            {
                MessageBox.Show("Please input fields: name, type, caption.");
