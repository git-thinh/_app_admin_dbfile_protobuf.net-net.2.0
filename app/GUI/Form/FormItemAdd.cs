...
namespace app.GUI
{
    ...
    public class FormItemAdd : FormBase, IDataFieldEvent
    {
            ...
            : base(true)
        {
            ...
            {
                var uc = new ucDataItemAdd(dbName, ki, fields[ki], _db, this) { Name = "uc" + ki.ToString(), };
        ...
        private void form_Submit(string dbName)
        {
                    ...
                    if (fi.Tag != null && fi.Tag.GetType() == m_TypeField)
                    {
                        ...
                        switch (fo.KIT)
                        {
                            ...
                            case ControlKit.HTML: // TextBox 
                                string html = (fi as HtmlEditor).Html;
                                Regex regx = new Regex(@"<body[^>]*>([\s\S]*?)</body>", RegexOptions.IgnoreCase | RegexOptions.Multiline);
                                Match match = regx.Match(html);
                                if (match.Success)
                                {
                                    string htm = match.Groups[1].Value;
                                    val = htm == null ? string.Empty : htm.Trim();
                                }
                                break;
                        ...
                        {
                            if (fo.FUNC_EDIT != null && fo.FUNC_EDIT.Contains(dbFunc.VALIDATE_EMPTY))
                            {
            ...
            if (data.Count > 0)
            {
                if (OnSubmit != null) OnSubmit(dbName, data);
                else
                {
