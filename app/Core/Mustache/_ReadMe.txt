# mustache#
    ...
    public class oBranch
    {
        ...
        public int type { set; get; }
            ...
            string tem = @"<ul>
{{#each Items}}
    <li class=""list-item{{#index}}"" value=""{{branch_id}}"">{{name}}</li>
{{/each}}
</ul>";
            htmlTemplate.Text = tem;

            HtmlFormatCompiler compiler = new HtmlFormatCompiler();
            var generator = compiler.Compile(tem);
			var list = new List<oBranch>() { };
            string tem_render = generator.Render(new { Items = list });
